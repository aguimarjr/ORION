...
Este README está organizado da sequinte forma:

1. **Resumo**: Visão geral do projeto ORION e seus objetivos
2. **Estrutura do readme.md**: Descrição da organização deste repositório
3. **Selos Considerados**: Artefatos e critérios de avaliação
4. **Informações básicas**: Organização do projeto e módulos
5. **Dependências**: Requisitos de hardware e software
6. **Preocupações com segurança**: Considerações de segurança
7. **Instalação**: Passo a passo para configuração do ambiente
8. **Teste mínimo**: Instruções para executar teste rápido
9. **Experimentos**: Procedimento para reproduzir resultados do artigo
10. **LICENSE**: Informações sobre licença do projeto
...
Por padrão, os arquivos de carga utilizados no artigo já se encontram gerados nas pastas correspondentes.
...
Os módulos `.py` que compõem este projeto de ambiente VEC (presentes na pasta `system/src/`) são descritos a seguir:
...
A execução do projeto não implica em questões de segurança.
...
## Reinvindicação 1: "O NSGA-II é o melhor algoritmo de otimização multiobjetivo para o contexto de VEC considerado"
